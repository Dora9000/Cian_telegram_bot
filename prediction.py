...
class Predictor:
    ...
    def preprocess(self, data, update=False):
        ...
        if update:
            data = save_column_format(data)
        else:
            data = data.reindex(columns=self.col_order)  # check the existence & need & order of columns

        data = data.apply(lambda x: x.fillna(0 if self.data_format[x.name] == self.INT_CNST else ''))
        data = encode_timestamps(data)
        data = self.categorical_encode(data)
        return data, y
    ...
    def predict_x(self, params):
        ...
        x_new = self.preprocess(data)[0]
        return abs(self.model.predict(x_new)[0])
    ...
    def predict_xs(self, min_edge, max_edge, file=DATA_FILE, test=False):
        ...
        x_new, y_new = self.preprocess(data)
        ...
        if test:
            print('original: ', y_new.tolist()[:10])
            print('predicted: ', y_predicted[:10])
            print('score: ', self.model.score(x_new, y_new))
